...
class AccountServiceImplTest {
    ...
    @Test
    public void itShouldCreateTransferWhenHeDepositZero() {
        //given
        Account account = new Account();
        account.setName("Zuppa");
        account.setSurname("Di Culo");
        ...
        //when
        accountService.deposit(new String(), zeroAmount);
        Mockito.verify(accountRepository).save(accountArgumentCaptor.capture());

        //then
        Account accountUpdated = accountArgumentCaptor.getValue();
        assertNotNull(accountUpdated);
        assertEquals(accountUpdated.getAmount(), account.getAmount());
        assertEquals(accountUpdated.getTransactions().size(), 1);
    }
    ...
    public void itShouldAddTransferWhenDoTransfer() {
        ...
        Mockito.when(transactionService.transfer(ArgumentMatchers.eq(accountFrom),
                        ArgumentMatchers.eq(accountTo),
                        ArgumentMatchers.eq(amountUpdated),
                        ArgumentMatchers.eq(Transaction.Type.EXTERNAL)))
                .thenReturn(transaction);
    ...
    @Test
    public void itShouldCreateTransferWhenDoTransferZero() {
        //given
        Account accountTo = new Account();
        accountTo.setId("To");
        accountTo.setName("Zuppa");
    ...
    @Test
    public void itShouldCreateDoubleTransferWhenAccountsAreTheSame() {
        //given
        Account account = new Account();
        account.setId("To");
        account.setName("Zuppa");
        ...
        TransferDto transferDto = new TransferDto();
        transferDto.setToAccountId(account.getId());
        transferDto.setFromAccountId(account.getId());
        transferDto.setAmount(amountUpdated);

        Transaction transaction = new Transaction();
        Mockito.when(accountRepository.findByIdAndActiveTrue(ArgumentMatchers.same(account.getId())))
                .thenReturn(Optional.ofNullable(account));
        Mockito.when(transactionService.transfer(ArgumentMatchers.eq(account),
        ...
        //then
        Account accountPostUpdate = accountArgumentCaptor.getAllValues().get(1);
        assertNotNull(accountPostUpdate);
        assertEquals(accountPostUpdate.getAmount(), amount);
        assertEquals(accountPostUpdate.getTransactionsSent().size(), 1);
        assertEquals(accountPostUpdate.getTransactionsReceived().size(), 1);
        assertEquals(accountPostUpdate.getTransactions().size(), 2);
    }
